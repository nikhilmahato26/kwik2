...
import { deleteHolidayPackage } from "@/app/admin/actions";
import DeleteButton from "@/components/admin/DeleteButton";

export const dynamic = 'force-dynamic';

export default async function AdminHolidays() {
  const holidayPackages = await db.holidayPackage.findMany({
                    ...
                    <tr key={pkg.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                        <div className="flex items-center justify-end gap-3">
                          <Link href={`/admin/holidays/${pkg.id}/edit`} className="text-gray-400 hover:text-indigo-600 transition-colors">
                            <Edit2 className="size-4" />
                          </Link>
                          <DeleteButton
                            action={deleteHolidayPackage}
                            id={pkg.id}
